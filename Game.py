...
gameOverFont=pygame.font.Font('./Assets/PixelFont1.ttf',50)
gameOverRestartFont=pygame.font.Font('./Assets/PixelFont1.ttf',25)
blink=30
...
gameOverSpace=gameOverFont.render('Game Over',False,'White')
gameOverRect=gameOverSpace.get_rect(center=(200,300))
gameOverRestart=gameOverRestartFont.render('Press \'R\' to continue',False,'White')
gameOverRestartRect=gameOverRestart.get_rect(center=(200,350))
...
while True:  # game loop
    ...
    if state:
        ...
        display.fill('Black')
        display.blit(gameOverSpace,gameOverRect)
        if(blink>=10):
            display.blit(gameOverRestart,gameOverRestartRect)
        blink-=1
        speed=0.4
        if(blink<5):
            blink+=30
